...
lapop2004_2018 <- lapop2004_2018 %>% 
  mutate(estatus_ocup = case_when((colocup4a <= 2 | ocup4a <= 2) & ocup1a == 3 ~ 1,
         ...
         estatus_ocup_f = factor(estatus_ocup, labels = c("Patrón", "Asalariado público",
                                                          "Asalariado privado",
...
variables_mca <- lapop2004_2018 %>% 
  select(ideologia_f, ed_f, estatus_ocup_f, ing_decil_f)
...
lapop2004_2018 %>% 
  ggplot(aes(x=as.factor(wave), y=democracia7_imp, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
  labs(title = "Democracia como mejor forma de gobierno por año y país", 
       subtitle = "Variable democracia7 imputada",
       y = "Aprobación de la democracia",
       caption = "Elaboración propia en base a LAPOP 2008-2018") +
  theme(plot.title = element_text(size = 10, face = "bold"),
        plot.subtitle = element_text(size = 9),
...
Pareciera que para 2016, en México, la variable de democracia imputada mejora la putuación hacia arriba.  
...
lapop2004_2018 %>% 
  ggplot(aes(x=as.factor(wave), y=reduc_desigualdad_imp, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
  labs(title = "Acuerdo con reducción de la desigualdad entre ricos y pobres por año y país", 
       subtitle = "Variable reduc_desigualdad imputada",
       y = "Aprobación de reducción de la desigualdad",
       caption = "Elaboración propia en base a LAPOP 2008-2018") +
  theme(plot.title = element_text(size = 10, face = "bold"),
        plot.subtitle = element_text(size = 9),
...
En el caso de la preferencia por la reducción de la desigualdad se producen cambios en México y Perú.  
...
desempleados <- lapop2004_2018 %>% 
  ...
  ggplot(aes(x=country_f, y=desempleados, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
...
ayuda_pobres <- lapop2004_2018 %>% 
  ...
  ggplot(aes(x=country_f, y=ayuda_pobres, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
...
impuestos_ricos <- lapop2004_2018 %>% 
  filter(wave == 2018) %>% 
  drop_na(ayuda_pobres) %>% 
  ggplot(aes(x=country_f, y=impuestos_ricos, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
...
lapop2004_2018 %>%
  ...
  group_by(country_f, wave, estatus_ocup_f) %>% 
  tally() %>%
  group_by(country_f, wave) %>% 
  mutate(porcentaje = n/sum(n)) %>% 
  ggplot(aes(x=as.factor(wave), y=porcentaje, fill= estatus_ocup_f)) +
  geom_col() +
...
lapop2004_2018 %>% 
  drop_na(democracia7, estatus_ocup_f) %>% 
  ggplot(aes(x=estatus_ocup_f, y=democracia7, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
...
lapop2004_2018 %>% 
  drop_na(reduc_desigualdad, estatus_ocup_f) %>% 
  ggplot(aes(x=estatus_ocup_f, y=reduc_desigualdad, fill= country_f)) +
  geom_boxplot(outlier.shape = NA, show.legend = FALSE) +
...
lapop2004_2018 %>% 
  drop_na(desempleados, estatus_ocup_f) %>% 
  group_by(country_f, estatus_ocup_f) %>% 
...
lapop2004_2018 %>% 
  drop_na(ayuda_pobres, estatus_ocup_f) %>% 
  group_by(country_f, estatus_ocup_f) %>% 
...
lapop2004_2018 %>% 
  drop_na(impuestos_ricos, estatus_ocup_f) %>% 
  group_by(country_f, estatus_ocup_f) %>% 
