...
pacman::p_load(tidyverse, kableExtra, GDAtools, FactoMineR, VIM, janitor, gridExtra,
               janitor, readxl, gtsummary, lme4, texreg, reghelper, nlme)
...
lapop2004_2018 <- lapop2004_2018 %>%
  mutate(redist2_inv = ifelse(wave == 2018, car::recode(redist2, "1=7; 2=6; 3=5; 4=4; 5=3; 
         ...
         formal_estado = ifelse(wave == 2018, case_when(formal == 1 ~ 1,
                                   formal == 2 ~ 2,
                                   categoria_ocup >= 5 ~ 3,
                                   TRUE ~ NA_real_), NA),
         formal_estado = factor(formal_estado, labels = c("Formal",
                                                                               "Informal",
                                                                               "No ocupados")),
...
democracia_ind_grup_int <- lmer(democracia7 ~ 1 + patron_cwc + asal_pub_cwc + asal_pri_cwc + 
                         cuenta_prop_cwc + desocupado_cwc + ideologia_cwc + sexo_cwc +
                         edad_cwc + educ_cwc + desempleo_cgm + informalidad_cgm +
                         empleo_pub_cgm + brecha_remun_cgm + desocupado_cwc*desempleo_cgm + (1 | pais_anio), 
                         data = base_ml)
...
htmlreg(list(democracia_ind_grup, democracia_ind_grup_int, democracia_random_apub,
             democracia_random_apri, democracia_random_cuent, democracia_random_desoc),
        custom.model.names = c("Efectos fijos", "Efectos fijos + int", "Random_apub", "Random_apri", "Random_cuent", "Random_desoc"),
        caption = "Democracia",
        caption.above = T,
...
anova(democracia_null, democracia_ind_grup)
...
varcomp_1=as.data.frame(VarCorr(democracia_ind_grup))
tau00_1=varcomp_1[1,4]
sigma2_1=varcomp_1[2,4]

varcomp_2=as.data.frame(VarCorr(democracia_ind_grup_int))
tau00_2=varcomp_2[1,4]
sigma2_2=varcomp_2[2,4]

varcomp_3=as.data.frame(VarCorr(democracia_random_apub))
tau00_3=varcomp_3[1,4]
sigma2_3=varcomp_3[2,4]

varcomp_4=as.data.frame(VarCorr(democracia_random_apri))
...
El pseudo r2 para el segundo nivel (países años) en el modelo con efectos fijos + interacción es de **`r R2_grup_m2`**, el del modelo de pendiente aleatoria para empleo público es de **`r R2_grup_m3`** y en el de empleo privado es de **`r R2_grup_m3`**.
...
reduc_desigualdad_ind_grup <- lmer(reduc_desigualdad  ~ 1 + patron_cwc + asal_pub_cwc + asal_pri_cwc + 
                         cuenta_prop_cwc + desocupado_cwc + ideologia_cwc + sexo_cwc +
                         edad_cwc + educ_cwc + desempleo_cgm + informalidad_cgm +
                         empleo_pub_cgm + brecha_remun_cgm + (1 | pais_anio), 
                         data = base_ml)
...
htmlreg(list(reduc_desigualdad_ind, reduc_desigualdad_ind_grup, reduc_desigualdad_random_apub,
             reduc_desigualdad_random_apri, reduc_desigualdad_random_cuent),
        custom.model.names = c("Ind","Ind+grup","Random_apub", "Random_apri", "Random_cuent"),
...
anova(reduc_desigualdad_ind_grup, reduc_desigualdad_random_apub)
anova(reduc_desigualdad_ind_grup, reduc_desigualdad_random_apri)
anova(reduc_desigualdad_ind_grup, reduc_desigualdad_random_cuent)
